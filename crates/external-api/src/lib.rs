...
impl<S: Storage> ExternalApi<S> {
    ...
    pub fn start(mut self, token: CancellationToken) -> JoinHandle<()> {
        ...
        tokio::spawn(async move {
            ...
            tokio::select! {
                r = ExternalApi::run(addr, state, api_state, auth, cfg, ip_mgr, mode, token) => {
                    if let Err(err) = r {
...
mod handlers {
    ...
    use ip_manager::{IpManager, OperationRecord, OperationStatus, Storage};
    use ipnet::Ipv4Net;
    ...
    use crate::models::{
        ActionResult, ConfigDocument, DrainRequest, Health, HealthResponse, Histogram,
        HistogramBucket, MaintenanceModeRequest, MetricFamily, MetricSample, MetricsDetailed,
        MetricsSummary, OpenMetricsJson, Operation, OperationAccepted, OperationLinks,
        PaginationMeta, ProtocolMetricsSummary, ReadinessCheck, ReadinessResponse, ReadinessStatus,
        ReserveIp, ServerApiInfo, ServerInfo, ServerResult, ShutdownRequest, State, V4Lease,
        V4LeaseListResponse, V4Reservation, V4ReservationListResponse, V6Lease,
        V6LeaseListResponse, V6Reservation, V6ReservationListResponse,
    };
